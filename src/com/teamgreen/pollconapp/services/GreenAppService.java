...
public class GreenAppService implements Serializable
{
    ...
    public boolean authenticateUser(String role, String userId, String password)
    {
        ...
        try
        {
            ...
            Query query = session.createQuery("from User where userId = :userid and password = :passwd and role = :role");
    ...
    {
        List<Owner> list = getSessionFactory().openSession().createQuery("from Owner  where aadharNumber=?").setParameter(0, aadhar).list();
    ...
    {
        List<Vehicle> list = getSessionFactory().openSession().createQuery("from Vehicle  where vehicleNumber=?").setParameter(0, selectedVehicleNumber).list();
